...
import { updateProductAction, getCategories } from '@/app/action/productService';
import Image from 'next/image';

export default function EditProductModal({ isOpen, onClose, product, refreshData }) {
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4 font-sans">
      <div className="bg-white rounded-[2rem] w-full max-w-4xl overflow-hidden shadow-2xl flex flex-col max-h-[95vh] border border-slate-100">
        ...
        <div className="flex justify-between items-center p-8 border-b border-slate-50 bg-[#fdfbfb]">
          <div>
            <h2 className="text-2xl font-normal text-slate-800 tracking-tight">Edit Masterpiece</h2>
        ...
        <form onSubmit={handleSubmit} className="p-8 overflow-y-auto flex flex-col gap-8 custom-scrollbar">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1.5 md:col-span-2">
                ...
                <input type="text" name="name" defaultValue={product.name} required className={inputStyles} />
              </div>
              <div className="space-y-1.5 md:col-span-2">
                <label className="text-xs font-medium text-slate-500 ml-1">Story / Description</label>
                <textarea name="description" defaultValue={product.description} rows="4" className={`${inputStyles} resize-none font-light leading-relaxed`} />
              </div>
            </div>
          </div>
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input type="number" name="price" defaultValue={product.price} required className={inputStyles} />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-slate-500 ml-1">Compare Price</label>
                  <input type="number" name="compare_price" defaultValue={product.compare_price} className={inputStyles} />
                </div>
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-slate-500 ml-1">Inventory Stock</label>
                <input type="number" name="stock" defaultValue={product.stock} required className={inputStyles} />
              </div>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <select name="category_id" defaultValue={product.category_id || ''} className={`${inputStyles} appearance-none cursor-pointer pr-10`}>
              ...
              <div className="space-y-1.5">
                ...
                <input type="text" name="tags" defaultValue={product.tags?.join(', ')} className={inputStyles} />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 border-t border-slate-50 pt-8">
            ...
            <textarea name="details" defaultValue={product.details?.join('\n')} rows="3" className={`${inputStyles} resize-none font-light`} />
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[0, 1, 2, 3].map((i) => (
                <div key={i} className="group relative aspect-[3/4] border-2 border-dashed border-slate-200 rounded-3xl flex items-center justify-center overflow-hidden hover:border-indigo-300 hover:bg-slate-50 transition-all duration-500 cursor-pointer">
                  {/* Hidden input to keep old URL if not replacing */}
                  {initialImages[i] && <input type="hidden" name={`existing_image${i + 1}`} value={initialImages[i]} />}
