        ...
        <div className="h-20 flex items-center justify-between px-8 border-b border-[#e5e5e5]">
          <Link href="/dashboard" className="block relative w-[120px] h-[35px] hover:opacity-70 transition-opacity duration-300">
            <Image 
              ...
              alt="Asaya Admin" 
              fill 
              sizes="120px" /* 🌟 ADDED SIZES PROP HERE */
  ...
  {link.name}
        ...
        <div className="p-6 border-t border-[#e5e5e5]">
          <div className="bg-white cursor-pointer p-4 flex items-center gap-4 hover:bg-[#fce3de] hover:border-[#1a1a1a]/30 transition-colors cursor-default">
            ...
            {/* 🌟 Dynamic Avatar: Grabs the first letter of the email, defaults to 'A' */}
            <div className="w-8 h-8 bg-[#1a1a1a] text-white flex items-center justify-center text-[12px] rounded-full font-bold uppercase shrink-0">
              {user?.email ? user.email.charAt(0).toUpperCase() : 'A'}
            </div>
            
            <div className="flex flex-col overflow-hidden">
              {/* 🌟 Safe fallbacks so the UI always looks perfect */}
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#1a1a1a] truncate">
                {user?.user_metadata?.name || 'admin@asaya.com'}
              </span>
              <span className="text-[9px] text-[#888] tracking-wider mt-0.5">
